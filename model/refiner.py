...
class Refiner(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        assert mode in ['full', 'sampling', 'thresholding','fastfull']
    ...
    def forward(self,
        ...
        H_base, W_base = pha.shape[2:]

        src_bgr = torch.cat([src, bgr], dim=1)

        if self.mode == 'sampling' or self.mode == 'thresholding':
            ...
            if self.kernel_size == 3:
                # The pad function generated by torch.onnx.export has an indefinite size and cannot be converted to tensorRT,
                # but it can be used by molding it with onnx-simplifier.
                x = F.pad(x, (3, 3, 3, 3))
                y = F.pad(y, (3, 3, 3, 3))
            ...
            if self.mode == 'fastfull':
                x = F.interpolate(x, (H_full, W_full), mode='bilinear', align_corners=False)
