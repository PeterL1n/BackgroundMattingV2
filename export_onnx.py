...
Example:
    ...
    python3 -m onnxsim export_model.onnx export_model-Simplify.onnx
...
parser.add_argument('--model-refine-mode', type=str, default='full', choices=['full', 'sampling', 'thresholding','fastfull'])
...
# gen model name w/params
root, ext = os.path.splitext(args.output)
if(args.model_refine_mode) == "sampling":
    refine_mode_name = str(args.model_refine_mode) + str(args.model_refine_sample_pixels)
elif(args.model_refine_mode) == "thresholding":
    refine_mode_name = str(args.model_refine_mode) + str(args.model_refine_threshold)
else:
    refine_mode_name = str(args.model_refine_mode)
...
print(f'ONNX model saved at: {output_name}')
...
if args.validate:
    ...
    sess = onnxruntime.InferenceSession(output_name)
